...
class DomainResourceIT {
    ...
    void getAllDomainsWithEagerRelationshipsIsEnabled() throws Exception {
        when(domainRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restDomainMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(domainRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllDomainsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(domainRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restDomainMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(domainRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
